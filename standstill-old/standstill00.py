...
class Standstill(tkinter.Tk):
    ...
    def norsk_documentation(self):
        ...
        self.language = 'no'
        self.cursor.execute(f"INSERT INTO standstillUser (language) VALUES ('{self.language}')")
    ...
    def tkinter_countdown(self):
        ...
        if self.counter < 0: 
            # Start listening audio and destroy canvas 
    ...
    def listening_audio(self, duration=5):
    ...
    def listening_silence(self, duration=5):
    ...
    def get_feedback(self):
        ...
        if not self.try_again:
            ...
            self.age_entry.place(relx=0.5, rely=0.8, anchor='center')
    ...
    def display_scores(self):
        ...
        if self.music_score == 0.0 or self.silence_score == 0.0:
            self.cursor.execute(f"DELETE FROM standstillUser WHERE standstillUserID = {self.id}")
            self.database.commit()
            self.cursor.execute(f"DELETE FROM standstillRealTime WHERE standstillUserID = {self.id}")
            self.database.commit()
